...
DATA=$ROOT/iwslt17.de_fr_zh.en.bpe16k
mkdir -p "$ORIG" "$DATA"
...
TEST_SETS=(
    ...
    "IWSLT17.TED.tst2010.zh-en IWSLT17.TED.tst2011.zh-en IWSLT17.TED.tst2012.zh-en IWSLT17.TED.tst2013.zh-en IWSLT17.TED.tst2014.zh-en IWSLT17.TED.tst2015.zh-en"
)
...
for ((i=0;i<${#SRCS[@]};++i)); do
    ...
    for ((j=0;j<${#TEST_SET[@]};++j)); do
        python "$SPM_ENCODE" \
            --model "$DATA/sentencepiece.bpe.model" \
            --output_format=piece \
            --inputs $DATA/test${j}.${SRC}-${TGT}.${SRC} $DATA/test${j}.${SRC}-${TGT}.${TGT} \
            --outputs $DATA/test${j}.bpe.${SRC}-${TGT}.${SRC} $DATA/test${j}.bpe.${SRC}-${TGT}.${TGT}
    done
done
